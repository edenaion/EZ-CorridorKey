...
> **Latest release: [v2.1.3](https://github.com/edenaion/EZ-CorridorKey/releases/tag/v2.1.3):** Windows hotfix. Fixes AI alpha generation for user folders with accented or non-Latin characters ([#184](https://github.com/edenaion/EZ-CorridorKey/issues/184)), repairs the CLI installer's FFmpeg check, and links the new video tutorial from the installation guide. See the [full changelog](CHANGELOG.md).
...
> Applies to v2.0.0 and source installs. 2.1.3 is a Windows-only patch, so there is no macOS update this release; source installs can still update with `3-update.sh`.
