...
> **最新リリース: [v2.1.4](https://github.com/edenaion/EZ-CorridorKey/releases/tag/v2.1.4):** Windows 向けのホットフィックスです。以前のバージョンがインストールした欠陥のある FFmpeg ビルドが原因で、インポート後に AI アルファ生成が「failed to read frame」で失敗する問題を修正しました ([#184](https://github.com/edenaion/EZ-CorridorKey/issues/184))。インポートは自動再試行付きの整合性チェックが行われるようになり、デフォルトでオフのオプションのクラッシュレポート機能が追加されました。[完全な changelog](../../CHANGELOG.md) を参照してください。
...
![EZ-CorridorKey](../screenshots/EZ-CorridorKey_Inference_Done.jpg)
...
[![Gumroad でダウンロード](https://img.shields.io/badge/Download-Gumroad-ff90e8?style=for-the-badge&logo=gumroad&logoColor=white)](https://edenaion.gumroad.com/)
...
> v2.0.0 とソースインストールに適用されます。2.1.4 のようなパッチリリースはアプリ内自動更新として提供され、新しい Mac インストーラーは含まれません。
...
![品質比較](../screenshots/quality_comparison_v1.5.0.png)
...
EZ-CorridorKey はデフォルトでテレメトリーを送信しません。使用状況の分析やトラッキングは一切行いません。

データを送信できるのは以下の 2 つのみで、いずれもご自身で制御できます。

☼ **問題を報告する**: ヘルプメニューからレポートを送信する際、ダイアログに表示される診断サマリーを開発者に直接送信することもできます。レポートごとに表示されるチェックボックスで制御します。
☼ **クラッシュレポート**: Preferences > Privacy のオプションのトグル。デフォルトではオフです。有効にすると、クラッシュの詳細が自動的に送信されます。

レポートに含まれる情報: クラッシュの詳細、GPU/ドライバー情報、OS とアプリのバージョン。含まれない情報: メディア、ファイル名、プロジェクト、個人情報。ファイルパスは送信前に匿名化されます。
...
[![スポンサー](https://img.shields.io/badge/GitHub-Sponsor-ea4aaa?style=for-the-badge&logo=githubsponsors&logoColor=white)](https://github.com/sponsors/edenaion)
...
[![EZSCAPE プラグイン](https://img.shields.io/badge/EZSCAPE-Plugins%20%26%20Tools-50FF80?style=for-the-badge&labelColor=000000)](https://www.ezscape.space)
